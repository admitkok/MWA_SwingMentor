<x-site-layout title="Featured swings">
    ...
    @foreach($posts as $post)
        ...
        <li class="shadow-lg shadow-gray-100/30 bg-white hover:shadow-lg rounded-lg grid grid-rows-2 grid-cols-1 md:grid-rows-1 overflow-hidden justify-end items-end">

            <a href="{{route('posts.show', ['post' => $post])}}" class="h-full bg-red-50 rounded-l overflow-hidden">
                <video hight="auto" width="700" autoplay muted  class="rounded-l h-full group-hover:scale-110 transition duration-500 cursor-pointer object-cover ">
                       <source src="{{$post->media->first() ? $post->media->first()->getUrl() : "demo.mp4"}}" type="video/mp4">
                </video>
                <div class="text-black"> adsom</div>
            </a>
