<x-site-layout title="{{$post->title}}">
    ...
    @foreach($post->mentors as $mentor)
        <a href="{{route('mentors.show', ['id' => $mentor->id])}}" class="bg-gradient-to-r from-blue-500/90 via-green-600/90 to-green-500/90 mb-4 text-white rounded-full py-1 px-4 text-sm">
            {{$mentor->name}}
        </a>
    @endforeach
    </div>
