<nav x-data="{ mobile_menu_visible: false }" class="bg-black border-b border-gray-800">
                    ...
                    <div class="ml-10 flex items-baseline space-x-4">
                        ...
                        @foreach($menu_items as $item)
                            @if(request()->routeIs($item['route']))
                                <a href="{!! $item['url'] ?? route($item['route']) !!}" class="font-medium font-mono text-white rounded-md px-3 py-2 text-sm" aria-current="page">{{$item['label']}}</a>
                            @else
                                <a href="{!! $item['url'] ?? route($item['route']) !!}" class="text-gray-300 font-light hover:bg-gray-700 hover:text-white font-mono  rounded-md px-3 py-2 text-sm">{{$item['label']}}</a>
                            @endif
                        @endforeach
                    </div>
                </div>
...
{{--                    </button>--}}
                    ...
                    @guest
                        <div class="font-mono text-gray-300">
                            <a href="{{route('register')}}" class="hover:bg-white/20 rounded text-gray-300 tracking-normal font-light text-base py-2 px-3">Register</a>
                        </div>


                        <div class="font-mono text-gray-300">
                            <a href="{{route('login')}}" class="bg-gradient-to-r from-blue-500/90 via-green-600/90 to-green-500/90 hover:bg-white text-white rounded-md tracking-normal font-light text-base py-2 px-3">LogIn</a>
                        </div>
...
{{--                            <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-1">Settings</a>--}}
    ...
    <div class="md:hidden" id="mobile-menu" x-show="mobile_menu_visible">
        <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
            @foreach($menu_items as $item)
                @if(request()->routeIs($item['route']))
                    <a href="{!! $item['url'] ?? route($item['route']) !!}" class="bg-gray-900 text-white block rounded-md px-3 py-2 text-base font-medium" aria-current="page">{{$item['label']}}</a>
                @else
